...
const CARDS_CONFIG = {
  ...
  consultor:           { icon: '⬟', titulo: 'Consultor ET2023', desc: 'Consulta interactiva de expedientes ET2023',           path: '/modules/consultor/consultor.html' },
...
if (usuario) {
  ...
  grid.innerHTML = allowed.filter(id => CARDS_CONFIG[id]).map(id => {
    ...
    return `
      <div class="module-card" onclick="${c.path.startsWith('http') ? `window.open('${c.path}','_blank')` : `window.location.href='${c.path}'`}">
